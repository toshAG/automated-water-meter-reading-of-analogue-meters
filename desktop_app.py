...
class MeterReaderApp:
    ...
    def _setup_style(self):
        ...
        style.configure("HeaderSub.TLabel", background=COLOR_HEADER, foreground="#cfe0ff", font=("Helvetica", 10))
        ...
        style.configure("ReadingUnit.TLabel", background=COLOR_PANEL, foreground=COLOR_TEXT_MUTED,
                         font=("Helvetica", 13))
        style.configure("SectionHeader.TLabel", background=COLOR_PANEL, foreground=COLOR_HEADER,
                         font=("Helvetica", 11, "bold"))
        style.configure("CardTitle.TLabel", background=COLOR_BG, foreground=COLOR_HEADER,
                         font=("Helvetica", 10, "bold"))

        style.configure("TButton", padding=(10, 7), font=("Helvetica", 10))
        style.map("TButton", background=[("active", "#e3ecff")])
        style.configure("Accent.TButton", padding=(12, 8), font=("Helvetica", 10, "bold"))
        style.map("Accent.TButton",
                  background=[("!disabled", COLOR_ACCENT), ("active", COLOR_ACCENT_DARK)],
                  foreground=[("!disabled", "#ffffff")])
        style.configure("TProgressbar", background=COLOR_ACCENT, troughcolor="#e0e4ea", thickness=6)
    ...
    def _build_layout(self):
        header = ttk.Frame(self.root, padding=(16, 13), style="Header.TFrame")
        header.pack(fill=tk.X)
        title_box = ttk.Frame(header, style="Header.TFrame")
        title_box.pack(side=tk.LEFT)
        ttk.Label(title_box, text="💧 Analog Meter Reader", style="Header.TLabel").pack(anchor="w")
        ttk.Label(title_box, text="Machine Learning Technique Based on Optical Character Recognation For Automated Reading of Analogue Meters", style="HeaderSub.TLabel").pack(anchor="w")

        self.camera_status_var = tk.StringVar(value="●  Camera: not opened")
        ttk.Label(header, textvariable=self.camera_status_var, style="HeaderSub.TLabel").pack(side=tk.RIGHT, anchor="e")

        main = ttk.Frame(self.root, padding=(14, 12, 14, 14))
        ...
        controls_card = ttk.Frame(left, style="Card.TFrame", padding=(12, 10))
        controls_card.pack(fill=tk.X, pady=(0, 8))

        ttk.Label(controls_card, text="CAPTURE", style="CardTitle.TLabel").pack(anchor="w", pady=(0, 6))
        controls_row1 = ttk.Frame(controls_card, style="Panel.TFrame")
        ...
        self.rotate_right_btn.pack(side=tk.LEFT)

        ttk.Separator(controls_card, orient="horizontal").pack(fill=tk.X, pady=(0, 8))

        ttk.Label(controls_card, text="ACTIONS", style="CardTitle.TLabel").pack(anchor="w", pady=(0, 6))
        controls_row2 = ttk.Frame(controls_card, style="Panel.TFrame")
        ...
        self.save_btn = ttk.Button(controls_row2, text="💾  Save Image", command=self.save_annotated_image, state=tk.DISABLED)
        ...
        self.status_label = ttk.Label(left, textvariable=self.status_var, style="Muted.TLabel", wraplength=700)
        ...
        preview_container = ttk.Frame(left, style="Card.TFrame")
        ...
        self._preview_placeholder_id = self.preview_canvas.create_text(
            300, 150, text="📷  Preview will appear here", fill="#8896ab", font=("Helvetica", 13)
        )

        # --- Right: stage-by-stage results + history ---
        right_outer = ttk.Frame(main, width=380)
        right_outer.pack(side=tk.RIGHT, fill=tk.Y, padx=(14, 0))
        right_outer.pack_propagate(False)

        right = ttk.Frame(right_outer, style="Panel.TFrame", padding=16)
        right.pack(fill=tk.BOTH, expand=True)

        ttk.Label(right, text="FINAL READING", style="SectionHeader.TLabel").pack(anchor="w")
        reading_row = ttk.Frame(right, style="Panel.TFrame")
        reading_row.pack(anchor="w", pady=(2, 10))
        self.reading_var = tk.StringVar(value="—")
        ttk.Label(reading_row, textvariable=self.reading_var, style="Reading.TLabel").pack(side=tk.LEFT)
        ttk.Label(reading_row, text=" m³", style="ReadingUnit.TLabel").pack(side=tk.LEFT, anchor="s", pady=(0, 6))
        ...
        self.stage_frame.pack(fill=tk.X, pady=(8, 10))
        ...
        self.history_list = tk.Listbox(history_frame, font=("Courier", 9), background=COLOR_PANEL,
                                        ...
                                        borderwidth=0, highlightthickness=1, highlightbackground=COLOR_BORDER)
    ...
    def _stat_row(self, parent, label, var_name, default):
        ...
        ttk.Label(row, text=f"{label}", width=18, style="Panel.TLabel",
                  foreground=COLOR_TEXT_MUTED, font=("Helvetica", 9)).pack(side=tk.LEFT)
        var = tk.StringVar(value=default)
        setattr(self, var_name, var)
        value_label = tk.Label(row, textvariable=var, font=("Helvetica", 11, "bold"),
    ...
    def open_camera(self):
        def do_open():
            try:
                ...
                self.result_queue.put(("status_error", str(e)))
                self.result_queue.put(("camera_open_failed", None))
    ...
    def upload_image(self):
        ...
        def do_load():
            try:
                ...
                self.result_queue.put(("status_info", f"Loaded {os.path.basename(path)}. Rotate if needed, then click Run Inference."))
    ...
    def _show_stage_thumbnails(self, result: dict):
        ...
        ttk.Label(self.stage_frame, text="Odometer crop", style="Panel.TLabel",
                  foreground=COLOR_TEXT_MUTED, font=("Helvetica", 9)).pack(anchor="w")
        odometer_photo = crop_to_photoimage(result.get("odometer_crop"), target_width=280)
        if odometer_photo is not None:
            self._stage_thumbnails.append(odometer_photo)
            tk.Label(self.stage_frame, image=odometer_photo, background=COLOR_PANEL,
                     highlightthickness=1, highlightbackground=COLOR_BORDER).pack(anchor="w", pady=(4, 10))

        pointer_crops = result.get("pointer_crops", [])
        if pointer_crops:
            ttk.Label(self.stage_frame, text=f"Pointer dials ({len(pointer_crops)}), left to right",
                      style="Panel.TLabel", foreground=COLOR_TEXT_MUTED, font=("Helvetica", 9)).pack(anchor="w")
            dials_row = ttk.Frame(self.stage_frame, style="Panel.TFrame")
            dials_row.pack(anchor="w", pady=(4, 4))
            for i, crop in enumerate(pointer_crops):
                photo = crop_to_photoimage(crop, target_width=76)
                if photo is not None:
                    self._stage_thumbnails.append(photo)
                    col = ttk.Frame(dials_row, style="Panel.TFrame")
                    col.pack(side=tk.LEFT, padx=(0, 6))
                    tk.Label(col, image=photo, background=COLOR_PANEL,
                    ...
                    tk.Label(col, text=str(digit), anchor="center", background=COLOR_ACCENT,
                             foreground="#ffffff", font=("Helvetica", 10, "bold"), width=4).pack(fill=tk.X, pady=(2, 0))
...
def main():
    ...
    parser.add_argument("--led_pin", type=int, default=None,
                         help="GPIO pin number for an LED light source turned on during each capture")

    parser.add_argument("--correct_image", action="store_true",
                         help="Apply gray-world white balance + CLAHE contrast + gamma correction to every captured frame.")
